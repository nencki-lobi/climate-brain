...
jobfile = {fullfile(basedir, 'code/first-level/batch_cet_job.m')};
...
for crun = 1:nrun
    ...
    mkdir(fullfile(resdir, sub, 'cet-model'))

    bold = 'space-MNI152NLin2009cAsym_desc-preproc_bold';

    inputs{1, crun} = {fullfile(resdir, sub, 'cet-model')}; % fMRI model specification: Directory - cfg_files
